...
- Окно отображения на гранулярность (значения по умолчанию, поправить при реализации если не подходят):
  - `minute` — как сейчас (24ч показ / 48ч кэш, без изменений).
  - `hour` — например 30 суток (720 точек).
  - `day` — без ограничения (бэк и так шлёт всё, объём тривиален).
- `METRICS_TICK_INTERVAL_MINUTES` (текущее `360`) — становится зависимым от выбранной гранулярности (тики реже для крупного шага, чтобы не захламлять ось).
