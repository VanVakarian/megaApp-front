const METRIC_DESCRIPTIONS: Record<string, Record<string, string>> = {
  hardware: {
    ...
    cpu_iowait_ratio_max:
      'Пиковая доля ожидания диска внутри минуты. Полезно для коротких, но заметных IO-всплесков.',
    cpu_steal_ratio_avg:
    ...
    disk_free_bytes: 'Сколько свободного места осталось на корневом диске сервера.',
    uptime_seconds:
    ...
    process_cpu_ratio_avg:
      'Средняя доля общей CPU-мощности сервера, которую за минуту съел сам процесс Flatline.',
  ...
  'spread-capture-bot-v3': {
    ...
    estimated_account_value:
      'Оценочная полная стоимость торгового счёта в USD: свободные деньги (free_cash) плюс то, сколько реально можно было бы получить, если бы бот сейчас продал все купленные позиции по текущим ценам в стакане. Это не «бумажная» оценка по последней сделке, а консервативная оценка по реальному спросу в стакане — поэтому она честнее показывает, сколько денег можно реально достать.',
    catalog_candidates:
  ...
  megaapp: {
    ...
    food_coefficients_job_ran:
      'Показывает, что в эту минуту отработала фоновая ежедневная задача пересчёта коэффициентов для продуктов в каталоге — она запускается по расписанию один раз в сутки. Значение 1 означает, что задача запустилась; в остальные минуты суток эта метрика просто не приходит, и это нормально.',
    backup_job_ran:
